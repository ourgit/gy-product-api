...
//            }
    ...
    private void setSkuFromParam(JsonNode node, ProductSku param, ProductSku productSku) {
        if (null != param && null != productSku) {
            ...
            if (node.has("enable")) productSku.setEnable(param.enable);
            if (node.has("stock")) productSku.setStock(param.stock);
            if (node.has("weight")) productSku.setWeight(param.weight);
            if (node.has("warningStock")) productSku.setWarningStock(param.warningStock);
            if (node.has("oldPrice")) productSku.setOldPrice(param.oldPrice);
            if (node.has("price")) productSku.setPrice(param.price);
            if (node.has("grouponPrice")) productSku.setGrouponPrice(param.grouponPrice);
            if (node.has("flashPrice")) productSku.setFlashPrice(param.flashPrice);
            if (node.has("distPrice")) productSku.setDistPrice(param.distPrice);
            if (node.has("award")) productSku.setAward(param.award);
            if (node.has("indirectAward")) productSku.setIndirectAward(param.indirectAward);
            if (node.has("recommendPrice")) productSku.setRecommendPrice(param.recommendPrice);
            if (node.has("minOrderAmount")) productSku.setMinOrderAmount(param.minOrderAmount);
            if (node.has("soldAmount")) productSku.setSoldAmount(param.soldAmount);
            if (node.has("virtualAmount")) productSku.setVirtualAmount(param.virtualAmount);
            if (node.has("sort")) productSku.setSort(param.sort);
            if (node.has("source")) productSku.setSource(param.source);
            if (node.has("beginTime")) productSku.setBeginTime(param.beginTime);
            if (node.has("endTime")) productSku.setEndTime(param.endTime);
            if (node.has("requireType")) productSku.setRequireType(param.requireType);
            if (node.has("status")) productSku.setStatus(param.status);
            if (node.has("deliverHours")) productSku.setDeliverHours(param.deliverHours);
            if (node.has("limitAmount")) productSku.setLimitAmount(param.limitAmount);
            if (node.has("presaleStock")) productSku.setPresaleStock(param.presaleStock);
            if (node.has("serviceAmount")) productSku.setServiceAmount(param.serviceAmount);
            if (node.has("serviceDays")) productSku.setServiceDays(param.serviceDays);
            if (node.has("requireScore")) productSku.setRequireScore(param.requireScore);
            if (node.has("requireInvites")) productSku.setRequireInvites(param.requireInvites);
            if (node.has("bidPrice")) productSku.setBidPrice(param.bidPrice);
            if (node.has("memberFavor")) productSku.setMemberFavor(param.memberFavor);
